"""
Minimal model.py for submission.
- Defines ARIMABaseline (from sample_submission/arima.py code).
- Exposes init_model() that returns a ready-to-infer model.
- Loads config from model_weights.pkl if present (dummy config in baseline).
"""
...
def init_model(weights_path: str | None = "model_weights.pkl") -> ARIMABaseline:
    ...
    if cfg:
        return ARIMABaseline(**cfg)
    return ARIMABaseline(order=(1, 1, 0), maxiter=50)